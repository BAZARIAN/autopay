...
curcontent["block"] = {
	xhead: 'Блокировка доната после вайпа', 
	xcon: '<div class="xbox_custom_block"><div class="kit-items">'
	
};

var BlockListArrays = [
["pistol.revolver","shotgun.waterpipe"],
["shotgun.double","pistol.python","pistol.semiauto","pistol.prototype17","pistol.m92","coffeecan.helmet","roadsign.jacket","roadsign.kilt"], 
["shotgun.pump","shotgun.spas12","smg.2","smg.thompson","smg.mp5","rifle.semiauto","rifle.m39", "multiplegrenadelauncher"],
["hmlmg","lmg.m249","rifle.ak","rifle.ak.ice","rifle.lr300","rifle.bolt","rifle.l96","grenade.f1","metal.facemask","metal.plate.torso","heavy.plate.helmet","heavy.plate.jacket","heavy.plate.pants"], 
["rocket.launcher","explosive.timed","explosive.satchel","grenade.beancan","ammo.rifle.explosive","ammo.grenadelauncher.he"]];
...
function generateUrlQiwi(){
	var publicKey = '48e7qUxn9T7RyYE1MVZswX1FRSbE6iyCj2gCRwwF3Dnh5XrasNTx3BGPiMsyXQFNKQhvukniQG8RTVhYm3iP6fG2fP54QnbHuVwzgLYNKseQwiNLEL1EQtFSc1b36V6KMNQp2emgxZvywWDdC1spadyY36HGdLtUfQyf1xuottMsfnDAKuGoY8SAHqyDc';
	var commentQiwi = `${CustomerSteamId}`;
	var successUrlQiwi = 'https://rustion.ru';
	var inputval = document.getElementById('refill-window__amount-input').value;
	var inputfloat = parseFloat(inputval);
	return 'https://oplata.qiwi.com/create?amount='+inputfloat+'&comment='+commentQiwi+'&publicKey='+publicKey+'&account=0&customFields[themeCode]=ALEKSEY-GHm9pjKFDa&successUrl='+successUrlQiwi;
}
...
function generateUrlEnot(){
	var inputval = document.getElementById('refill-window__amount-input').value;
	var MERCHANT_ID   = 33090,             // ID магазина
	SECRET_WORD   = 'OL_7QONHr4OKOaK8CNh9kxkPWfUXHe_w',   // Секретный ключ
...
window.onload = function () {
	...
	for (var i = 0; i < BlockListArrays.length; i++) {
		...
		for (var b = 0; b < BlockListArrays[i].length; b++) {
			if(b == 5 && i == 4)
			curcontent["block"].xcon += '<div class="kit-items__item"><div class="kit-item"><img class="kit-item__image" src="https://moscow-cdn.ru/rust/items/icons256/ammo.grenadelauncher.he.png">\
...
function appcentHandler() {
	...
	fetch('https://cent.app/api/v1/bill/create', {
		...
		body: JSON.stringify({
			...
			name: `Your SteamID64 - ${CustomerSteamId}`,
		})
	})
		.then((res) => {
			if (res.ok) {
